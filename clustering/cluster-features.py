...
from sklearn.preprocessing import scale, robust_scale, minmax_scale, maxabs_scale, RobustScaler
...
if __name__ == "__main__":
   ...
   # features_data = maxabs_scale(features_data)
   #features_data = PCA(n_components="mle", whiten=True).fit_transform(features_data)
   features_data = minmax_scale(features_data)
   ...
   for i in range(num_clusters):
      c = centroids
      plt.text(c[i, 0]+0.01, c[i, 1]+0.01, "[" + str(i) + "]", color="white")
   ...
   if os.path.isdir("output") == False:
      os.mkdir("output")
   with open(os.path.join("output", "clustering-" + str(seed) + "-" + str(num_clusters) + ".csv"), "w") as fid:
   ...
   plt.savefig(os.path.join("output", "clustering-" + str(seed) + "-" + str(num_clusters) + ".pdf"))
   plt.show()
